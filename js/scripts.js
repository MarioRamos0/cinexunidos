    ...
    if (contenedorOpcionesCine) {
      ...
      nuevoDato
        ...
        .addEventListener("click", () => {
          ocultarPeliculas();
          ocultarAsientos();
          mostrarAuditorios(dato.auditoriums, dato.id);
        });
    } else {
...
function mostrarAuditorios(auditoriums, cineId) {
  ...
  auditoriums.forEach((auditorio) => {
    ...
    nuevaFilaAuditorio
      ...
      .addEventListener("click", () => {
        ocultarAsientos();
        mostrarSalas(auditorio.showtimes, cineId, auditorio.id);
      });
  });
}
...
function mostrarSalas(salas, cineId, auditorioId) {
  ...
  //tablaSalas.style.display = "block";
  tablaSalas.classList.remove("hidden");
  ...
  salas.forEach((sala) => {
    ...
    nuevaFilaSala
      .getElementsByTagName("button")[0]
      .addEventListener("click", () => {
        mostrarAsientos(sala, cineId, auditorioId);
      });
  });
}
...
function mostrarAsientos(dato, cineId, auditorioId) {
  ...
  selectedMovieElement.textContent =
    "Película seleccionada: " + dato.movie.name;
...
function verficacionReservacionAsiento(ruta) {
  rutaAux = ruta + `/reservation-updates`;

  console.log("Ruta de revisar reservacion:", rutaAux);
  fetch(rutaAux, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
// Función para ocultar la tabla de películas y la de asientos
function ocultarPeliculas() {
  const tablaPeliculas = document.getElementById("tabla-salas-container");
  tablaPeliculas.classList.add("hidden");
}
